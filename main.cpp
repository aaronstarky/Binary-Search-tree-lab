...
string BSTtoString(BST* bst) {
	...
	while (!readQ.empty()) { // as long as the queue has a remaining node:
		...
		for (; i > 0; i--) { // for each node on this level,
			...
			if (nextNode->getLeftChild() != NULL) { // if there is a leftChild child, push the leftChild child into the queue
				readQ.push(nextNode->getLeftChild());
			}
			if (nextNode->getRightChild() != NULL) { // if there is a rightChild child, push the leftChild child into the queue
